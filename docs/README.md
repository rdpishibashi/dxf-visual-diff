...
3. **オフセット補正設定** (オプション):
   - 基準点の違いによる誤検知を減らすためのオフセット補正
   - `analyze_offset.py` で事前に分析したオフセット値を使用
   - 詳細は [OFFSET_COMPENSATION_GUIDE.md](OFFSET_COMPENSATION_GUIDE.md) を参照

4. **ファイル処理**: 「DXF差分を比較」ボタンをクリックして比較開始

5. **結果のダウンロード**: 正常に処理された各ペアごとにダウンロードボタンを表示
...
### analyze_offset.py - オフセット分析ツール

DXFファイル間の座標オフセットを分析し、基準点の違いを検出するツールです。
...
詳細は [QUICK_START.md](QUICK_START.md) および [OFFSET_COMPENSATION_GUIDE.md](OFFSET_COMPENSATION_GUIDE.md) を参照してください。

## バージョン履歴

- **v1.1.0**: オフセット補正機能の追加
  - `analyze_offset.py` による事前分析機能
  - ファイルBへのグローバルオフセット適用
  - 基準点の違いによる誤検知の削減
  - ペアごとの個別オフセット設定
