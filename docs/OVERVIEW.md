...
| 症状 | 対処 |
|------|------|
| 「DXFファイルの読み取りエラー」 | ファイルが有効なDXF形式であることを確認。破損またはパスワード保護されていないか確認 |
| 「メモリエラー」 | ファイルペアの数を減らす。エンティティ数の少ないファイルを使用する |
| 「処理タイムアウト」 | 大きい・複雑なファイルは処理に時間がかかる。座標許容誤差を増やして処理時間を短縮することを検討 |
...
| バージョン | 内容 |
|-----------|------|
| v1.1.0 | オフセット補正機能の追加（`analyze_offset.py` による事前分析、ファイルBへのグローバルオフセット適用、基準点の違いによる誤検知の削減、ペアごとの個別オフセット設定） |
| v1.0.0 | 初回リリース。最大5ファイルペアのサポート、高精度比較エンジン、色分けされた差分可視化、ファイル名の自動生成 |
