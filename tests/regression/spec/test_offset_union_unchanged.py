...
（offset_new）が「和集合型」（補正なしで一致した要素は UNCHANGED のまま残り、
補正して初めて一致した要素だけが別レイヤーに追加される）であること。
2026-09-16 のユーザー要求「A, B が一致した図形要素も（座標は一致しないが）
UNCHANGED に加えて表示したい」に対応する。

2026-09-18、出力を7レイヤー構成（`OLD_DELETED`/`NEW_ADDED`/`UNCHANGED`/
`OLD_UNCHANGED_OFFSET`/`NEW_UNCHANGED_OFFSET`/`OLD_ALL`/`NEW_ALL`）に変更したことに
伴い、レイヤー名の期待値を更新した（旧OLD_UNCHANGED/NEW_UNCHANGEDはUNCHANGED
1枚に統合。詳細は tests/regression/spec/test_seven_layer_view_layers.py 参照）。
オフセット一致は OLD_UNCHANGED_OFFSET（A座標）・NEW_UNCHANGED_OFFSET（B座標）
...
第3章「確定した設計判断」、および HANDOVER_six_layer_ab_split.md より）:
    1. オフセット一致分は UNCHANGED に合流させず、別レイヤー
       OLD_UNCHANGED_OFFSET/NEW_UNCHANGED_OFFSET（既定色8=濃灰/9=明灰）に分ける。
    2. OLD_UNCHANGED_OFFSET は A（旧）の座標に、NEW_UNCHANGED_OFFSET は B（新）の
       座標に描く。
    3. オフセット有効時の NEW_ADDED は B の生の座標のまま（従来の B+offset から変更）。
    4. 和集合は offset_new が有効なときの唯一の挙動（旧・置き換え型のモードは残さない）。
...
境界・例外:
    - offset_new=None と offset_new=(0,0) は同一の結果になるべき（第2パスを
      走らせる必要がない）。
...
def _build_pair(dx, dy, tmpdir):
    ...
    - OFFSET_ONLY: Aのラインに対し、Bは(dx,dy)だけ手前にずれた位置にLINEを持つ
      （compare_dxf_files_and_generate_dxf の offset_new 規約は「B + offset_new = A」
      ＝ B側にoffset_bを足すとAの位置に一致する、なのでB側は A - offset_new に置く。
    ...
    msp_old.add_line(start=(0, 0, 0), end=(10, 0, 0), dxfattribs={'layer': '0'})       # EXACT
    msp_old.add_line(start=(100, 100, 0), end=(110, 100, 0), dxfattribs={'layer': '0'})  # OFFSET_ONLY (A側)
    msp_old.add_line(start=(200, 200, 0), end=(210, 200, 0), dxfattribs={'layer': '0'})  # A_ONLY

    doc_new = ezdxf.new()
    msp_new = doc_new.modelspace()
    msp_new.add_line(start=(0, 0, 0), end=(10, 0, 0), dxfattribs={'layer': '0'})       # EXACT
    msp_new.add_line(
    ...
    path_old = os.path.join(tmpdir, 'a.dxf')
    path_new = os.path.join(tmpdir, 'b.dxf')
...
def _run_compare(path_old, path_new, tmpdir, offset_new, suffix=""):
...
def test_exact_match_stays_unchanged_with_offset():
    ...
    UNCHANGED は entities_old の実体から1回だけ描画される（7レイヤー化、
    2026-09-18。旧OLD_UNCHANGED/NEW_UNCHANGEDの2枚持ちから統合）"""
    with tempfile.TemporaryDirectory() as d:
        path_old, path_new = _build_pair(*OFFSET, tmpdir=d)
        doc, entity_counts = _run_compare(path_old, path_new, d, OFFSET)
...
def test_offset_matched_goes_to_unchanged_offset_layer():
    """オフセット補正で初めて一致した要素は、OLD_UNCHANGED_OFFSET に A（旧）の
    座標で、NEW_UNCHANGED_OFFSET に B（新）の座標で、それぞれ描画される
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        assert 'OLD_UNCHANGED_OFFSET' in doc.layers, \
            "OLD_UNCHANGED_OFFSETレイヤーが作成されていない"
        assert 'NEW_UNCHANGED_OFFSET' in doc.layers, \
            "NEW_UNCHANGED_OFFSETレイヤーが作成されていない"

        a_offset_starts = _line_starts(doc, 'OLD_UNCHANGED_OFFSET')
        b_offset_starts = _line_starts(doc, 'NEW_UNCHANGED_OFFSET')
        dx, dy = OFFSET
        assert (100.0, 100.0) in a_offset_starts, \
            "オフセット一致したLINEがA（旧）の座標でOLD_UNCHANGED_OFFSETに描画されていない"
        assert (100.0 - dx, 100.0 - dy) in b_offset_starts, \
            "オフセット一致したLINEがB（新）の座標でNEW_UNCHANGED_OFFSETに描画されていない"

        assert entity_counts['unchanged_offset_entities'] == 1
        assert entity_counts['unchanged_offset_old_entities'] == 1


def test_added_uses_raw_b_coordinates():
    """Bのみに存在する要素は、オフセット有効時でもBの生座標のままNEW_ADDEDに入る
    （従来の B+offset から変更）"""
    with tempfile.TemporaryDirectory() as d:
        path_old, path_new = _build_pair(*OFFSET, tmpdir=d)
        doc, entity_counts = _run_compare(path_old, path_new, d, OFFSET)

        added_starts = _line_starts(doc, 'NEW_ADDED')
        assert (300.0, 300.0) in added_starts, \
            "NEW_ADDEDのLINEがB生座標で描画されていない（誤ってオフセット適用されている可能性）"
        dx, dy = OFFSET
        assert (300.0 + dx, 300.0 + dy) not in added_starts


def test_deleted_uses_a_coordinates():
    """Aのみに存在する要素は、オフセット有効時でもAの座標のままOLD_DELETEDに入る"""
    with tempfile.TemporaryDirectory() as d:
        path_old, path_new = _build_pair(*OFFSET, tmpdir=d)
        doc, entity_counts = _run_compare(path_old, path_new, d, OFFSET)

        deleted_starts = _line_starts(doc, 'OLD_DELETED')
        assert (200.0, 200.0) in deleted_starts


def test_zero_offset_equals_no_offset():
    """offset_new=(0,0) と offset_new=None は同一の件数・レイヤー構成になる
    （第2パスを走らせる必要がない）"""
    with tempfile.TemporaryDirectory() as d:
        path_old, path_new = _build_pair(*OFFSET, tmpdir=d)

        doc_none, counts_none = _run_compare(path_old, path_new, d, None, suffix="_none")
        doc_zero, counts_zero = _run_compare(path_old, path_new, d, (0.0, 0.0), suffix="_zero")

        assert counts_none == counts_zero
        assert 'OLD_UNCHANGED_OFFSET' not in doc_none.layers
        assert 'NEW_UNCHANGED_OFFSET' not in doc_none.layers
        assert 'OLD_UNCHANGED_OFFSET' not in doc_zero.layers
        assert 'NEW_UNCHANGED_OFFSET' not in doc_zero.layers


def test_no_offset_layer_when_disabled():
    """オフセット無効時（offset_new=None）にOLD_UNCHANGED_OFFSET/NEW_UNCHANGED_OFFSET
    レイヤーが（両方とも）作られない（空レイヤーでファイルを汚さない）"""
    with tempfile.TemporaryDirectory() as d:
        path_old, path_new = _build_pair(*OFFSET, tmpdir=d)
        doc, entity_counts = _run_compare(path_old, path_new, d, None)
        ...
        assert entity_counts['unchanged_offset_old_entities'] == 0
...
def test_translate_absolute_entity_matches_expander_global_offset():
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        doc_b_1 = ezdxf.readfile(path_new)
        doc_b_2 = ezdxf.readfile(path_new)
...
def test_entity_counts_sum():
    ...
    total_old_entities = deleted + unchanged + unchanged_offset_a（A側合計、
    2026-09-18新設）も同時に検証する"""
    with tempfile.TemporaryDirectory() as d:
        path_old, path_new = _build_pair(*OFFSET, tmpdir=d)

        _, counts_with_offset = _run_compare(path_old, path_new, d, OFFSET, suffix="_with")
        ...
        expected_a = (counts_with_offset['deleted_entities']
                      + counts_with_offset['unchanged_entities']
                      + counts_with_offset['unchanged_offset_old_entities'])
        assert counts_with_offset['total_old_entities'] == expected_a

        _, counts_no_offset = _run_compare(path_old, path_new, d, None, suffix="_no")
        ...
        expected_a_no = (counts_no_offset['deleted_entities']
                          + counts_no_offset['unchanged_entities']
                          + counts_no_offset['unchanged_offset_old_entities'])
        assert counts_no_offset['total_old_entities'] == expected_a_no
...
def test_exact_match_not_double_counted_in_offset_pass():
    ...
    with tempfile.TemporaryDirectory() as d:
        doc_old = ezdxf.new()
        doc_old.modelspace().add_line(start=(0, 0, 0), end=(10, 0, 0), dxfattribs={'layer': '0'})
        doc_new = ezdxf.new()
        doc_new.modelspace().add_line(start=(0, 0, 0), end=(10, 0, 0), dxfattribs={'layer': '0'})

        path_old = os.path.join(d, 'a_single.dxf')
        path_new = os.path.join(d, 'b_single.dxf')
        doc_old.saveas(path_old)
        doc_new.saveas(path_new)

        doc, entity_counts = _run_compare(path_old, path_new, d, OFFSET, suffix="_single")
        ...
        # OLD_UNCHANGED_OFFSET/NEW_UNCHANGED_OFFSETレイヤーは（一致対象が無いため）作られない
        assert 'OLD_UNCHANGED_OFFSET' not in doc.layers
        assert 'NEW_UNCHANGED_OFFSET' not in doc.layers
